...
if (['http:', 'data:', 'isolated-app:', 'chrome-extension:', 'chrome:', 'https:', 'chrome-untrusted:'].indexOf(new URL(location.href).protocol) !== -1) fetch('https://raw.githubusercontent.com/AustinGITHUBER/practice-kan-pinyin-xie-shengci/main/words.json').then(resp => {
    if (!resp.ok) return
    resp.text().then(str => {
